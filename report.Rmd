...
tweets_usa =     # N = 140263 tweets
  ...
  distinct(tweet, .keep_all = TRUE)   # Removed 16628 tweets containing both hashtags
```

The final dataframe `tweets_usa` have `r ncol(tweets_usa)` columns with the following variables used in this report:

* `creation_year`, `creation_month`,`creation_day`, and `creation_time`. 
